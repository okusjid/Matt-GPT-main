...
class OpenAIGpt(metaclass=Singleton):
    ...
    def generate(self, user_email, content):
        return self.generate_assistant(user_email, content)

    def generate_assistant(self, user_email, content):
        thread = self.create_thread(user_email)
        response = self.client.beta.threads.messages.create(
            thread.id,
            role="user",
            content=content,
        )
        messages = self.run_assistant(thread.id)
        ...
        while run.status == "in_progress" or run.status == "queued":
            ...
            if run.status == "requires_action":
                ...
                if tool_call.function.name == "send_user_query_as_email":
                    user_email = json.loads(tool_call.function.arguments)["user_email"]
                    user_name = json.loads(tool_call.function.arguments)["user_name"]
                    user_query = json.loads(tool_call.function.arguments)["user_query"]
                    user_interest = json.loads(tool_call.function.arguments)["user_interest"]
                    result = self.email_sender.send_user_query_as_email({
                        "user_name": user_name,
